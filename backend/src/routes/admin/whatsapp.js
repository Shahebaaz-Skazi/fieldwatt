  ...
  try {
    ...
    for (const propertyId of propertyIds) {
      try {
        ...
        results.push({ propertyId, phone: formattedPhone, status, error: apiError, url: selfReadingUrl, tokenUsedPrefix: (tokenVal || '').substring(0, 15) + '...', phoneIdUsed: finalPhoneId, envKeys: Object.keys(process.env).filter(k => k.includes('WHATSAPP')) });
