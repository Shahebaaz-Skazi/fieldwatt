#!/usr/bin/env python3
"""
Ultra-fast Ajmera meter images importer using Pillow & ThreadPoolExecutor.
...
def process_single_row(row_tuple):
    ...
    try:
        ...
        if asg:
            ...
            asg_id = f"asg_17sep_{prop_id[:8]}_{int(time.time()*1000)}"
            query_d1(
        ...
        # 6. Upload to R2
        r2_key = f"meter_photos/upload_17sep_{stripped_bp}_{int(time.time()*1000)}.jpg"
        try:
...
def main():
    excel_file = sys.argv[1] if (len(sys.argv) > 1 and sys.argv[1].strip()) else r'f:\fieldwatt\Upload 17.09.2026.xlsx'
    if not os.path.exists(excel_file):
        excel_file = 'Upload 17.09.2026.xlsx'
