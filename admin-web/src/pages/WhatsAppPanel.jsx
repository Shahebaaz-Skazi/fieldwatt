...
const WhatsAppPanel = () => {
  const [usage, setUsage] = useState({ sentThisMonth: 0, limit: 1000 });
  ...
  const filteredProperties = properties.filter(p => {
    ...
    if (pendingOnly && p.reading_status === 'reading_taken') return false;
    return true;
  });
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '20px' }}>
            ...
            />
            <label htmlFor="pending-only-check" style={{ fontSize: '13px', color: 'var(--text)', cursor: 'pointer' }}>
              Show Pending / Unread properties only
            </label>
          </div>
        </div>
