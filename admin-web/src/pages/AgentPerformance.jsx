  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: '700', color: 'var(--text)', margin: 0 }}>Agent Performance</h1>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '24px', flexWrap: 'wrap', alignItems: 'center' }}>
        {[
          ...
          <button
            ...
            style={{
              padding: '6px 16px', borderRadius: '20px', fontSize: '13px', cursor: 'pointer', fontWeight: period === opt.value ? '600' : '400',
              background: period === opt.value ? '#f5a623' : 'var(--surface)',
              color: period === opt.value ? '#000' : 'var(--text)',
              border: period === opt.value ? '1px solid #f5a623' : '1px solid var(--border)',
            }}
          >
            {opt.label}
          </button>
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {data?.agents?.map(agent => {
            ...
            return (
              <div key={agent.agent_id} style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '12px', overflow: 'hidden' }}>
                ...
                >
                  ...
                  <div style={{ flex: 2, minWidth: '160px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', color: 'var(--muted)', marginBottom: '4px' }}>
                      <span>{readingTaken} of {totalAssigned} done</span>
                      <span style={{ fontWeight: '600', color: completionPct >= 80 ? '#22c55e' : completionPct >= 50 ? '#f59e0b' : '#ef4444' }}>{completionPct}%</span>
                    </div>
                    <div style={{ height: '6px', background: 'var(--border)', borderRadius: '3px', overflow: 'hidden' }}>
                      <div style={{ height: '100%', width: `${completionPct}%`, background: completionPct >= 80 ? '#22c55e' : completionPct >= 50 ? '#f59e0b' : '#ef4444', borderRadius: '3px', transition: 'width 0.5s ease' }} />
                    </div>
                  </div>

                  {/* Quick Stats */}
                  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                    <span style={{ fontSize: '12px', padding: '3px 10px', borderRadius: '20px', background: 'rgba(34,197,94,0.1)', color: '#22c55e', fontWeight: '600' }}>✓ {readingTaken} Done</span>
                    <span style={{ fontSize: '12px', padding: '3px 10px', borderRadius: '20px', background: 'rgba(148,163,184,0.1)', color: '#94a3b8' }}>⏳ {notVisited} Pending</span>
                  </div>
                  ...
                  <div style={{ borderTop: '1px solid var(--border)', padding: '16px 20px', background: 'var(--card, var(--surface))' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '10px' }}>
                      {Object.entries(STATUS_CONFIG).map(([key, cfg]) => {
                        ...
                        return (
                          <div key={key} style={{ background: cfg.bg, border: `1px solid ${cfg.color}22`, borderRadius: '8px', padding: '12px', display: 'flex', flex: 'column' }}>
                            <div style={{ fontSize: '11px', color: cfg.color, fontWeight: '600', marginBottom: '4px' }}>{cfg.label}</div>
